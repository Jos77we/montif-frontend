...
const CardFetch = () => {
  ...
  return (
    ...
    {Array.isArray(results) && results.map((item, index) => (
        <div style={{marginTop:'10px', height:'200px', width:'90%', backgroundColor:'gold', borderRadius:'6px', marginLeft:'auto', marginRight:'auto'}} key={index}>
           ...
           <div style={{height:'60px', backgroundColor:'grey', borderBottomLeftRadius:'6px', borderBottomRightRadius:'6px', display:'flex', alignItems:'center'}}>
            ...
            <p style={{marginLeft:'60px', fontWeight:'600'}}>{item.cardType}</p>
            <p style={{fontWeight:'600'}}>{item.cardNo}</p>
            </div>
           </div>
        </div>
    ))}
